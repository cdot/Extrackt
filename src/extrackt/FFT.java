 ...
 *
 * This class has been derived from Bruce Miller's FFT package.
 * <p>
 * Complex data is represented by 2 double values in sequence: the real and
 * imaginary parts. Thus N data points are represented by a double array
 * dimensioned to 2*N. The physical layout in the array data, of the
 * mathematical data d[i] is as follows:
 * <PRE>
 *     Re(d[i]) = data[i0 + stride*i]
 *     Im(d[i]) = data[i0 + stride*i+1]
 * </PRE> The default offset, i0, is 0 and the stride is 2.
 * <p>
 * The transformed data is returned in the original data array in wrap-around
 * order.
 * <p>
 * The result of a Fourier transform of either real or complex data is always
 * complex, although in the real case, there is an extra symmetry. The
 * transformed data is left in the same array as the initial data. If D[i] is
 * the transform of d[i], where delta is the time spacing between points in the
 * original data d[i]:
 *
 * <pre>
 ...
 * .		...		..
 * n-3		d[t=(n-3)delta]	D[f=-3/(n delta)]
 * n-2		d[t=(n-2)delta]	D[f=-2/(n delta)]
 * n-1		d[t=(n-1)delta]	D[f=-1/(n * delta)]
 * </pre>
 *
 * You can convert to natural order (where the negative frequencies come first)
 * by using toNaturalOrder.
 *
 ...
 * @author Crawford Currie added interrupt ability
 */
public class FFT {
    ...
    /**
     * Constructs a complex FFT transformation for n complex data points. The
     * FFT way be interrupted by setting the interrupted flag. An interrupted
     * FFT will leave the data array in an inconsistent state.
     *
     * @param direction If FFT.FORWARD, compute the FFT of time-domain data,
     * storing the frequency domain result in wrap-around order. If BACKWARD
     * compute the reverse transform. normalise() must be called on the
     * resultant data to get the inverse of the FFT.
     * @param data [2*n] array of data, organised this: Re(D[i]) = data[i],
     * Im(D[i]) = data[i+1]. For an transform of real data, Im is 0.
     */
    public FFT(int direction, double data[]) throws InterruptedException {
        if (data.length % 2 == 1) {
            throw new IllegalArgumentException("The data array is uneven length");
        }
        this.data = data;
        n = data.length / 2;
        // SMELL: we never yield, so can this ever be set true?
        interrupted = false;
        setup_wavetable();
        transform_internal(0, 2, direction);
    }

    /**
     * Normalised the result of a BACKWARD transform to give the inverse FFT of
     * data.
     */
    ...
    /**
     * Reorder the transformed data from most negative frequency to most
     * positive frequency leaving the result in data.
     *
     * Divides by the number of data points to remove the FFTs scaling.
     */
    public void toNaturalOrder() {
    ...
    /**
     * Reorder the data to wraparound order. Multiplies by the number of points
     * to reverse the toNaturalOrder method.
     */
    public void toWrapAroundOrder() {
        double[] scratch = new double[2 * n];
        System.arraycopy(data, 0, scratch, 0, 2 * n); // save the data
        System.arraycopy(scratch, n + n % 2, data, 0, n - n % 2); // copy the second half of the data into the first half
        System.arraycopy(scratch, 0, data, n - n % 2, n + n % 2); // copy the first half of the data into the second half
    ...
    /**
     * Gets an array containing the frequencies in wrap-around order if samples
     * in the original data are separated by delta.
     *
     * @param delta
     * @return the array of frequencies
     */
    public double[] getWrappedDf(double delta) {
    ...
    /**
     * Gets an array containing the frequencies in natural order if samples in
     * the original data are separated by delta.
     *
     * @param delta
     * @return the array of frequencies
     */
    public double[] getNaturalDf(double delta) {
        double[] freq = new double[n];
        double f = -0.5 / delta, df = -2 * f / (n - n % 2);
        for (int i = 0; i < n; i++) {
            freq[i] = f;
    ...
    private final int available_factors[] = {7, 6, 5, 4, 3, 2};

    private void setup_wavetable() throws InterruptedException {
        factors = factor(n, available_factors);
        double d_theta = -2.0 * Math.PI / ((double) n);
        int product = 1;
     ...
     */
    private void transform_internal(int i0, int stride, int sign) throws InterruptedException {
        if (n == 1) {
            return;  // FFT of 1 data point is the identity
        }
        double localScratch[] = new double[2 * n];
        int product = 1;
        int state = 0;
        double in[], out[];
        int istride, ostride;
        ...
        if (state == 1) {  /* copy results back from localScratch to data */

            for (int i = 0; i < n; i++) {
                data[i0 + stride * i] = localScratch[2 * i];
                data[i0 + stride * i + 1] = localScratch[2 * i + 1];
            }
        }
    }
     ...
     * @param product
     */
    void pass_2(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 2;
        int m = n / factor;
        int q = n / product;
     ...
     * @param product
     */
    void pass_3(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 3;
        int m = n / factor;
        int q = n / product;
     ...
     * @param product
     */
    void pass_4(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 4;
        int m = n / factor;
        int q = n / product;
     ...
     * @param product
     */
    void pass_5(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 5;
        int m = n / factor;
        int q = n / product;
        ...
        double sin_2pi_by_5 = sign * Math.sin(2.0 * Math.PI / 5.0);
        double sin_2pi_by_10 = sign * Math.sin(2.0 * Math.PI / 10.0);
        int i = in0, j = out0;
        int di = istride * m;
        int dj = ostride * p_1;
        double x_real, x_imag;
     ...
     * @param product
     */
    void pass_6(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 6;
        int m = n / factor;
        int q = n / product;
     ...
     * @param product
     */
    void pass_7(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int product) throws InterruptedException {
        int k, k1;
        int factor = 7;
        int m = n / factor;
        int q = n / product;
        int p_1 = product / factor;
        int jump = (factor - 1) * p_1;
        double c1 = Math.cos(1.0 * 2.0 * Math.PI / 7.0);
        double c2 = Math.cos(2.0 * 2.0 * Math.PI / 7.0);
        double c3 = Math.cos(3.0 * 2.0 * Math.PI / 7.0);
        double s1 = (-sign) * Math.sin(1.0 * 2.0 * Math.PI / 7.0);
        double s2 = (-sign) * Math.sin(2.0 * 2.0 * Math.PI / 7.0);
        double s3 = (-sign) * Math.sin(3.0 * 2.0 * Math.PI / 7.0);
        int i = in0, j = out0;
        int di = istride * m;
        int dj = ostride * p_1;
        double x_real, x_imag;
     ...
     * @param product
     */
    void pass_n(int fi, double in[], int in0, int istride, double out[], int out0, int ostride, int sign, int factor,
            int product) throws InterruptedException {
        int i, j, k, k1;
        int m = n / factor;
        int q = n / product;
        int p_1 = product / factor;
